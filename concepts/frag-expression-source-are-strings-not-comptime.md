...
**orthogonal to timing** — to *when* the event runs. Representation and
...
way it survives at comptime." Comptime-ness is now **explicit**: an event is
...
event with an `is_expression`/`is_source` field (parser), and the emitter
...
of the runtime module, dropping the flow's own branch events (e.g. a `~for`
...
- KEPT conservative: `flowInvokesComptimeEvent`'s TypeRegistry fallback still
  ...
  comptime event not present in the local AST can't be re-classified there
  yet. An open edge.

**Blast radius = correctness.** 8 stdlib events leaned solely on the flip,
...
comptime events already declared it explicitly. Migration cost: events that
