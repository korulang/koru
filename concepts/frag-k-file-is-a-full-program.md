...
event declarations. KORU111 Rule 1 encoded that belief — it rejected a private
event in a `.k` as "dead syntax," on the reasoning that *no procs live in `.k`,
so nothing inside the file can call a private event.*

That reasoning is false, and the belief flipped: **a `.k` is a full program.**
Events implement as pure Koru right there in the file (subflow / immediate-impl,
not a host proc), and the file's own local flows call them — including private
ones. So a private event in a `.k` is not dead; it is ordinary internal
...
The ruling (Lars, 2026-07-17): **private events are legal in `.k`.** Rule 1 is
...
test once that enforcement exists — its `input.kz` calls a private event across
