...
## Closed 2026-09-01 — `koru_std/program_ast.zig` grandfather row

The allowlist's first row was a ~9,626-line AST serializer dump of
`compiler.kz` — header said "Generated by Koru AST Serializer", already
denied by `**/program_ast.zig`, and not imported anywhere. The live Stage-C
path writes `program.ast.json` at compile time (`main.zig`, `koru_std/build.zig`).
