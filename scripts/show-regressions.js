 ...
 *
 * Shows NEW regressions - tests that broke recently (last 1-3 snapshots).
 ...
 * - NEW regressions (broke in last 3 snapshots) - PRIORITY
 * - Long-standing failures (broke >3 snapshots ago) - may be architectural work
...
const RECENT_THRESHOLD = 3; // Snapshots within this count are "new regressions"
...
async function main() {
	...
	for (const failingTest of failingTests) {
		...
		if (lastPassing) {
			const timeSinceBroke = Date.now() - new Date(lastPassing.snapshot.timestamp).getTime();
			const regression = {
				...failingTest,
				...
				breakingCommit: lastPassing.snapshotsAgo > 0
					? snapshots[lastPassing.snapshotsAgo - 1].gitCommit
					: 'current'
			};

			if (lastPassing.snapshotsAgo < RECENT_THRESHOLD) {
	...
	if (recentRegressions.length > 0) {
		console.log(`NEW regressions: \x1b[1m\x1b[31m${recentRegressions.length} tests\x1b[0m (broke in last ${RECENT_THRESHOLD} snapshots)`);
	}
	console.log('');
	...
	if (recentRegressions.length > 0) {
		...
		for (const reg of recentRegressions) {
			...
			const justBroke = reg.snapshotsAgo === 0 ? ' \x1b[1m\x1b[41m JUST BROKE \x1b[0m' : '';
			console.log(`\x1b[31m❌ ${reg.directory}\x1b[0m${justBroke}`);
			console.log(`   Last passed: ${formatTimestamp(reg.lastPassedAt)} (commit ${reg.lastPassedCommit})`);
			console.log(`   Broken for: ${reg.snapshotsAgo + 1} snapshot${reg.snapshotsAgo > 0 ? 's' : ''} (${formatDuration(reg.timeSinceBroke)})`);
	...
	if (longStandingFailures.length > 0) {
		console.log(`\x1b[1m📅 LONG-STANDING FAILURES\x1b[0m (broke >${RECENT_THRESHOLD} snapshots ago - may indicate architectural changes):`);
		console.log('');

		for (const failure of longStandingFailures) {
			console.log(`\x1b[33m⚠️  ${failure.directory}\x1b[0m`);
			console.log(`   Last passed: ${formatTimestamp(failure.lastPassedAt)} (commit ${failure.lastPassedCommit})`);
			console.log(`   Broken for: ${failure.snapshotsAgo + 1} snapshots (${formatDuration(failure.timeSinceBroke)})`);
	...
	if (neverPassed.length > 0) {
		...
		for (const test of neverPassed) {
			console.log(`   \x1b[33m❔\x1b[0m ${test.directory} (${test.failureReason})`);
		}
		console.log('');
	}
	...
	if (recentRegressions.length > 0) {
		...
		console.log(`\x1b[1m🔥 PRIORITY: Fix NEW regressions (${recentRegressions.length} tests)\x1b[0m`);
		console.log(`   Start with: ${mostRecent.directory}`);
		console.log(`   Use git bisect between ${mostRecent.lastPassedCommit} (good) and ${mostRecent.breakingCommit} (bad)`);
		console.log('');
	}

	if (longStandingFailures.length > 0) {
		console.log(`📅 Long-standing failures (${longStandingFailures.length} tests)`);
		console.log('   May indicate ongoing architectural changes - lower priority');
		console.log('');
	}
	...
	if (recentRegressions.length > 0) {
		console.log(`\x1b[31m⚠️  ${recentRegressions.length} NEW regression${recentRegressions.length > 1 ? 's' : ''} need attention!\x1b[0m`);
		process.exit(1);
	} else if (longStandingFailures.length > 0 || neverPassed.length > 0) {
		console.log(`\x1b[33m⚠️  No NEW regressions, but ${longStandingFailures.length + neverPassed.length} old failure${longStandingFailures.length + neverPassed.length > 1 ? 's' : ''} remain\x1b[0m`);
		process.exit(1);
	} else {
