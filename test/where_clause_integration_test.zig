...
test "parse and generate code for when clause" {
    const allocator = testing.allocator;
    
    const source =
        \\~api.fetch(url: "https://api.example.com/data")
        \\| ok response when response.status == 200 |> json.parse(response.body)
        \\| error e |> log.error(e)
    ;
...
test "complex when clause with logical operators" {
    const allocator = testing.allocator;
    
    const source =
        \\~validate(input: data)
        \\| valid v when v.score > 90 and v.approved |> process(v)
        \\| invalid i when i.retryable or i.code == "TEMP_ERROR" |> retry(i)
        \\| invalid i |> reject(i)
    ;
...
test "where clause in proc context" {
    ...
    const source =
        ...
        \\    return ~http.get(url)
        \\    | ok o when o.status == 200 |> .{ success = o.body }
        \\    | ok o when o.status >= 500 |> .{ retry = .{} }
