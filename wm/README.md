...
- `producer/cordial.ts` — the regression WORLD-MODEL faucet: reads the same
  ...
  onto the **Cordial signals bus** (`:6285/signal`). Two families, split by the
  dumb-signal/smart-engine razor: `koru.regression.test` (RAW per-test FLIP cards,
  `route: surface, wmfx=regression_shape`) and `test-health` (the whole-run VERDICT
  `green|red|regression`, `route: surface, wake`). So individual tests AND the run
  result surface all the way to Cordial. `bun wm/producer/cordial.ts` — and it fires
  automatically at the end of `wm/run.sh`, so `wm run .` (governed adapter) closes
  the loop while `run_regression.sh` stays dumb.
