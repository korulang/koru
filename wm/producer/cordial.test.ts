...
import { verdictOf, flipCard, verdictCard, cardsFor, stateToPrev, runCard, type RunSummary } from "./cordial.ts";
...
console.log("\nrunCard (purely-visual trace beat):");
ok("started beat → koru.regression.run = started, surface-only family", () => {
  const c = runCard("started", "suite started @abcd123");
  assert.equal(c.name, "koru.regression.run");
  assert.equal(c.value, "started");
  assert.equal(c.source, "koru.regression");
  assert.ok(c.note.includes("abcd123"));
});
ok("trace beat → value 'trace' carrying the message in the note", () => {
  const c = runCard("trace", "compiling backend.zig");
  assert.equal(c.value, "trace");
  assert.equal(c.note, "compiling backend.zig");
});
