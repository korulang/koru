;;; N-body benchmark - SBCL kernel DSL variant
;;; By Robert Smith (stylewarning) - https://www.stylewarning.com/posts/nbody/
;;; Adapted for Koru language shootout output format (energy on stderr)
;;;
;;; Build with:
;;;   sbcl --noinform --no-userinit --no-sysinit \
;;;     --load "reference/nbody_kernel_lib.lisp" \
;;;     --load "reference/nbody_kernel.lisp" \
;;;     --eval '(sb-ext:save-lisp-and-die "bin/sbcl-kernel" :toplevel (function nsim:main) :purify t :executable t)'
...
(defun main ()
  ...
  (let ((n (parse-integer (second sb-ext:*posix-argv*))))
    ...
    (format *error-output* "~,9F~%" (energy *system*))
    (sb-ext:exit :code 0)))
