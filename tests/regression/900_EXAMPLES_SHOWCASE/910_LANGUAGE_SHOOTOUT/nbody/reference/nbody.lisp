;;; N-body benchmark - Common Lisp (SBCL) reference
;;; Run with: sbcl --script nbody.lisp <iterations>
;;; Build with: sbcl --noinform --non-interactive \
;;;   --eval '(compile-file "reference/nbody.lisp")' \
;;;   --eval '(load "reference/nbody.fasl")' \
;;;   --eval '(sb-ext:save-lisp-and-die "bin/sbcl-nbody" :toplevel (function main) :executable t)'
      ...
      (let ((xi (aref x i)) (yi (aref y i)) (zi (aref z i))
        ...
        (loop for j fixnum from (1+ i) below 5 do
          (let* ((dx (- xi (aref x j)))
                 ...
                 (mag (/ +dt+ (* dsq (sb-ext:truly-the double-float (sqrt dsq)))))
