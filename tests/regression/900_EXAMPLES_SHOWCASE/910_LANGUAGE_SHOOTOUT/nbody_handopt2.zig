// HAND-OPTIMIZED N-BODY v2: inline for instead of manual unroll
//
// Hypothesis: Manual unrolling caused code bloat, hurting icache.
// Try: inline for (comptime unroll) which lets compiler decide.
...
// Hot loop - let compiler decide on unrolling
fn advanceOnce(b: *Bodies) void {
    // Velocity updates - nested loop, compiler unrolls if beneficial
    for (0..5) |i| {
        for (i + 1..5) |j| {
            const dx = b.x[i] - b.x[j];
            const dy = b.y[i] - b.y[j];
