...
echo "📦 Compiling C reference (gcc -O3 -march=native -ffast-math)..."
gcc reference/reference.c -O3 -march=native -ffast-math -fno-math-errno -lm -o reference/nbody-c

echo "📦 Compiling Zig baseline (zig -O ReleaseFast -mcpu=native)..."
zig build-exe reference/baseline.zig -O ReleaseFast -mcpu=native -femit-bin=reference/nbody-zig
...
./run_regression.sh 2101_nbody_optimized > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "❌ ERROR: Koru compilation failed"
    echo "   Run './run_regression.sh 2101_nbody_optimized' to see errors"
    exit 1
fi
cd tests/regression/900_EXAMPLES_SHOWCASE/910_LANGUAGE_SHOOTOUT/2101_nbody_optimized
