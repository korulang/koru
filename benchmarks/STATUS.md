...
- **Workload: reduce_kernel** — Koru vs C vs Rust, scalar reduction of a 64-body kernel over step(0..n). Finding: **on-par with a reassociating C, by scope contract, not by a race.** Before the associative contract Koru sat at parity with strict C (1.641 vs 1.714/1.703 ns/elem, min). `std/kernel:reduce` is declared an **associative fold**, so its loop emits under `@setFloatMode(.optimized)` and LLVM reassociates (vectorize on fold-resistant data, fold on static). That is exactly what a `-ffast-math` C build does, so the honest comparison is parity with fast-math C — the apparent 5-14x "speedup" is a comparison against a strict C that is deliberately not allowed to reassociate. The real difference is **scope**: C must flip fast-math globally (impossible when part of the program must stay exact), Koru confines the win to the one contractually-associative op; `self`/`pairwise` stay strict. Composes under `step` (pinned `390_112_reduce_self_under_step`); 390_110/111 stay green. Trustworthy measurement is `scripts/trust.sh` (user-CPU seconds, min + median + spread); validated on a clean x86_64 droplet (wall ≈ user there).
...
- **Workload: regex_match** — 5 langs (Koru/Go/Rust/JS/Python), n=3M classification dispatches over rotating email/number/none inputs, full-match, first-match-wins. Patterns compile to specialized native DFA matchers at COMPILE TIME in Koru (`~match` + backtick branches); everyone else compiles at runtime (hoisted). Medians: Koru 45 ms, Rust 93 ms, JS 151 ms, Go 324 ms, Python 917 ms — identical output hashes. ~15 ns/dispatch for Koru (DFA table walk + branch dispatch).
- **Workload: regex_redos** — pathological `(a+)+b` vs `"a"×n`. Koru/Rust FLAT (≈26→30 ms wall incl. startup from n=30 to n=10,000,000 — the match itself never registers); Go flat-with-slope (380 ms at 10M); V8 and CPython go vertical: JS 39.7 s at n=30, Python 7.9 s at n=28. Koru full-matches ten million chars in less time than V8 needs to fail on thirty. Koru is flat BY CONSTRUCTION (compile-time DFA, no backtracking path to fall into); Go/Rust are flat by runtime engineering (RE2 lineage).
