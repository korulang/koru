...
DIR="$ROOT/workloads/$WL/$LANG"
[ -d "$DIR" ] || { echo "$WL,$LANG,$N,,,,,no-such-implementation"; exit 1; }
...
case "$LANG" in
    ...
    *)          echo "$WL,$LANG,$N,,,,,unknown-language"; exit 1 ;;
esac
...
echo "$WL,$LANG,$N,$WALL_MS,$OUTPUT_HASH,\"$OUTPUT_CLEAN\","
