...
Every example below is verbatim source from a passing POSITIVE regression test (negative MUST_ERROR tests are excluded — these are all what-to-do). NO PROSE: category and per-test prose have been removed — the tests are the only source of truth (prose drifts and contaminates). These are the receipts.
...
```koru
// TEST: fmt:ln basic — format a string and bind the result with `: l`.
import std/fmt
import std/io

tor greet { name: string } -> string

greet = std/fmt:ln("Hello, {{ name:s }}!"): l -> l.text
